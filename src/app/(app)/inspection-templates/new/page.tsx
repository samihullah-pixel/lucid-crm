  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="font-sans text-sm font-light text-grey">Name und Objekt/Standort festlegen, dann Bereiche und Punkte hinzufügen.</p>
      </div>
      <form action={createInspectionTemplate} className="max-w-lg space-y-4 border border-gold/20 bg-white p-6">
        <div>
          ...
          <input
            name="name"
            required
            placeholder="z.B. Bürokomplex – Wöchentliche Kontrolle"
            className="w-full rounded border border-black/15 px-3 py-2 font-sans text-sm focus:border-gold focus:outline-none"
          />
        </div>
        <div>
          <label className="mb-1 block font-sans text-[11px] uppercase tracking-wide text-grey">Standort (optional)</label>
          <select name="siteId" className="w-full rounded border border-black/15 px-3 py-2 font-sans text-sm focus:border-gold focus:outline-none">
            <option value="">— kein Standort zugeordnet —</option>
            {sites.map((s) => (
